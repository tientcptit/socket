...
void data_init()
{
    ...
    for (int i = 0; i < MAX_STUDENT; i++)
    {
        ...
        student[i].maths = rand() % 10 + (float)i / 4;
        student[i].physics = rand() % 10 + (float)i / 4;
        student[i].chemistry = rand() % 10 + (float)i / 4;
        student[i].biology = rand() % 10 + (float)i / 4;
        student[i].history = rand() % 10 + (float)i / 4;
        student[i].geography = rand() % 10 + (float)i / 4;
        student[i].literature = rand() % 10 + (float)i / 4;
        student[i].english = rand() % 10 + (float)i / 4;
    }
}
...
{
    pthread_t tid;
    pthread_create(&tid, NULL, listen_thread_func, NULL);
...
void *listen_thread_func(void *arg)
{
    ...
    while (1)
    {
        ...
        // pass to connect thread
        pthread_arg->connfd = connfd;
        pthread_arg->client_addr = client_addr;

        printf("Connection accepted from %s:%d\n", cl_addr, cl_port);

        // call connect thread to handle connection
        int th_stt = pthread_create(&tid, NULL, connect_thread_func, (void *)pthread_arg);
        pthread_join(tid, NULL);
        if (th_stt != 0)
        {
            perror("Connnect thread");
            exit(1);
        }
    }
}
